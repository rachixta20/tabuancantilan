...
<a href="{{ route('admin.users') }}" class="sidebar-link {{ request()->routeIs('admin.users*') ? 'active' : '' }}">
    ...
    Users
    @php try { $pendingSellers = \App\Models\User::where('role','farmer')->where('account_status','pending')->count(); } catch (\Exception $e) { $pendingSellers = 0; } @endphp
    @if($pendingSellers > 0)
        <span class="ml-auto badge bg-amber-100 text-amber-700">{{ $pendingSellers }}</span>
    @endif
</a>
...
<a href="{{ route('admin.reports') }}" class="sidebar-link {{ request()->routeIs('admin.reports*') ? 'active' : '' }}">
    ...
    Reports
    @php try { $pendingReports = \App\Models\Report::where('status','pending')->count(); } catch (\Exception $e) { $pendingReports = 0; } @endphp
    @if($pendingReports > 0)
        <span class="ml-auto badge bg-red-100 text-red-700">{{ $pendingReports }}</span>
    @endif
</a>
